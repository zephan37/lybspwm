...
void Widget::update_process_value()
{
    ...
    exec_linux_cmd(QString("pamixer --get-mute"), mute_status, cmd_error);
    exec_linux_cmd(QString("pamixer --get-volume"), cmd_result, cmd_error);

    if(mute_status == QString("true\n")) {
        ui->groupBox_3->hide();
        ui->label->setPixmap(QPixmap(":/resource/resource/image/mute.png"));

        current_value = mute_status;
    }
    else {
        ui->groupBox_3->show();
        ui->label->setPixmap(QPixmap(":/resource/resource/image/audio.png"));
